...
create table if not exists brands (
  ...
  logo_url text,
  website text,
  social jsonb,              -- {instagram:"", x:"", tiktok:""}
...
create table if not exists missions (
  ...
  sponsored_by_brand_id uuid references brands on delete set null, -- "Sponsored by"
  created_by uuid references auth.users on delete set null,
  created_at timestamptz default now()
);
...
create table if not exists submissions (
  ...
  media jsonb not null,        -- ör: [{path:"userid/123.jpg", type:"image"}]
  note text,
...
create table if not exists wallet_transactions (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users on delete cascade,
  amount int not null,          -- + kazanım, - harcama
...
create table if not exists reward_redemptions (
  ...
  code text,                    -- hediye kodu vs.
  created_at timestamptz default now()
);
alter table reward_redemptions enable row level security;

-- 9) Bildirimler (varsa atla, yoksa oluştur)
...
-- profiles
create policy if not exists "profiles select all" on profiles for select using (true);
create policy if not exists "profiles insert self" on profiles for insert with check (auth.uid() = id);
create policy if not exists "profiles update self" on profiles for update using (auth.uid() = id);

-- follows
create policy if not exists "follows select own relations" on follows
for select using (auth.uid() = follower_id or auth.uid() = following_id);
create policy if not exists "follows request" on follows
for insert with check (auth.uid() = follower_id and follower_id <> following_id);
create policy if not exists "follows cancel-unfollow" on follows
for delete using (auth.uid() = follower_id or auth.uid() = following_id);
create policy if not exists "follows accept" on follows
for update using (auth.uid() = following_id) with check (auth.uid() = following_id);

-- brands & locations
create policy if not exists "brands read public" on brands for select using (true);
create policy if not exists "brands manage creator" on brands for all using (auth.uid() = created_by) with check (auth.uid() = created_by);
create policy if not exists "brand_locations read public" on brand_locations for select using (true);
create policy if not exists "brand_locations manage creator" on brand_locations for all using (
  auth.uid() = (select created_by from brands b where b.id = brand_locations.brand_id)
) with check (
  auth.uid() = (select created_by from brands b where b.id = brand_locations.brand_id)
);

-- missions
create policy if not exists "missions read public" on missions for select using (true);
create policy if not exists "missions manage creator" on missions for all using (auth.uid() = created_by) with check (auth.uid() = created_by);

-- mission_favorites
create policy if not exists "fav select own" on mission_favorites for select using (auth.uid() = user_id);
create policy if not exists "fav insert own" on mission_favorites for insert with check (auth.uid() = user_id);
create policy if not exists "fav delete own" on mission_favorites for delete using (auth.uid() = user_id);

-- submissions
create policy if not exists "submissions read approved or owner or brand" on submissions
for select using (
  status = 'approved'
  or auth.uid() = user_id
  or auth.uid() in (select created_by from missions m join brands b on m.brand_id=b.id where m.id = submissions.mission_id)
);
create policy if not exists "submissions insert self" on submissions
for insert with check (auth.uid() = user_id);
create policy if not exists "submissions update owner or brand" on submissions
for update using (
  auth.uid() = user_id
  or auth.uid() in (select created_by from missions m join brands b on m.brand_id=b.id where m.id = submissions.mission_id)
);
...
-- likes & comments
create policy if not exists "likes select public" on submission_likes for select using (true);
create policy if not exists "likes insert self" on submission_likes for insert with check (auth.uid() = user_id);
create policy if not exists "likes delete self" on submission_likes for delete using (auth.uid() = user_id);

create policy if not exists "comments select public" on submission_comments for select using (true);
create policy if not exists "comments insert self" on submission_comments for insert with check (auth.uid() = user_id);
create policy if not exists "comments delete self" on submission_comments for delete using (auth.uid() = user_id);

-- wallet
create policy if not exists "wallet select own" on wallet_transactions for select using (auth.uid() = user_id);
-- Güvenli taraf: earn/spend işlemleri fonksiyon/trigger ile eklenecek. Gerekirse aşağıdaki insert'i kapalı tut.
create policy if not exists "wallet insert self spend-only" on wallet_transactions
for insert with check (auth.uid() = user_id);

-- rewards
create policy if not exists "reward read public" on reward_catalog for select using (status='active');
create policy if not exists "reward manage creator" on reward_catalog for all using (auth.uid() = created_by) with check (auth.uid() = created_by);

-- redemptions
create policy if not exists "redemption select own" on reward_redemptions for select using (auth.uid() = user_id);
create policy if not exists "redemption insert own" on reward_redemptions for insert with check (auth.uid() = user_id);
create policy if not exists "redemption update own pending" on reward_redemptions for update using (auth.uid() = user_id);

-- notifications
create policy if not exists "notifs select own" on notifications for select using (auth.uid() = user_id);
create policy if not exists "notifs insert own" on notifications for insert with check (auth.uid() = user_id);
create policy if not exists "notifs update own" on notifications for update using (auth.uid() = user_id);
...
-- =========================
-- NOT: Dashboard'da bu bucket'ları oluşturduktan sonra çalıştır.
...
create policy if not exists "avatars owner update" on storage.objects
for update to authenticated using (bucket_id='avatars' and owner = auth.uid())
with check (bucket_id='avatars' and owner = auth.uid());
create policy if not exists "avatars owner delete" on storage.objects
...
-- media JSON'unda path alanını object name ile aynı kaydettiğin için join mümkün.
create policy if not exists "subs read owner or approved" on storage.objects
for select using (
...
create policy if not exists "subs owner write" on storage.objects
for insert to authenticated with check (bucket_id='submissions' and owner = auth.uid());
create policy if not exists "subs owner update" on storage.objects
...
create policy if not exists "subs owner delete" on storage.objects
...
create policy if not exists "chat owner read" on storage.objects
for select using (bucket_id='chat' and owner = auth.uid());
create policy if not exists "chat owner write" on storage.objects
for insert to authenticated with check (bucket_id='chat' and owner = auth.uid());
create policy if not exists "chat owner update" on storage.objects
for update to authenticated using (bucket_id='chat' and owner = auth.uid())
with check (bucket_id='chat' and owner = auth.uid());
create policy if not exists "chat owner delete" on storage.objects
...
create policy if not exists "profiles owner insert" on profiles
