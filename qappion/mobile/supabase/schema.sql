...
-- Yayında ve aktif görevler (brand ile)
...
-- Liderlik tablosu (onaylı gönderiler)
create or replace view public.v_mission_leaderboard as
  select s.mission_id,
         ...
         s.caption,
         s.likes_count,
         row_number() over (partition by s.mission_id order by s.likes_count desc, s.created_at asc) as rank
  from public.submissions s
  where s.status = 'approved';

-- ====== INDEXES ======
create index if not exists idx_products_published on public.products(is_published, is_active);
create index if not exists idx_missions_published on public.missions(is_published, is_active, starts_at, ends_at);
