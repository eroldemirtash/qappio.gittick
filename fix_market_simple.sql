-- Basit çözüm: Market hatalarını düzelt
-- Bu script'i Supabase Dashboard > SQL Editor'da çalıştırın

-- 1. Brands tablosuna updated_at kolonu ekle
...
-- 4. Product marketplaces tablosunu oluştur (eğer yoksa)
...
DROP POLICY IF EXISTS "product_marketplaces_read" ON public.product_marketplaces;
...
-- 7. Test verisi ekle (eğer tablo boşsa)
INSERT INTO public.product_marketplaces (product_id, marketplace, url) 
SELECT 
  p.id,
  'Amazon',
  'https://amazon.com/' || p.title
FROM public.products p
WHERE p.is_active = true
AND NOT EXISTS (SELECT 1 FROM public.product_marketplaces WHERE product_id = p.id);
